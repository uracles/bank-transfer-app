...
public class TransactionSummaryController {
    ...
    public ResponseEntity<ApiResponse<TransactionSummaryResponse>> getDailySummary(
            @RequestParam(required = false)
            @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate date) {
        TransactionSummaryResponse summary = transferService.getSummaryForDate(
                date != null ? date : LocalDate.now());
        return ResponseEntity.ok(ApiResponse.success(summary));
    }

    @PostMapping("/generate")
    @Operation(summary = "Manually trigger daily summary generation for a specific date (defaults to yesterday)")
    public ResponseEntity<ApiResponse<TransactionSummaryResponse>> generateSummary(
            @RequestParam(required = false)
            @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate date) {
        TransactionSummaryResponse summary = transferService.generateDailySummaryAndFetch(
                date != null ? date : LocalDate.now().minusDays(1));
        return ResponseEntity.ok(ApiResponse.success("Summary generated for " + summary.getSummaryDate(), summary));
    }
}